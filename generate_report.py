...
def get_payment_conditions(conditions_path):
    """Legge le condizioni di pagamento da condizioni.xlsx."""
    payment_conditions = {}
    try:
        workbook = openpyxl.load_workbook(conditions_path, data_only=True)
        ...
        print(f"ERRORE in get_payment_conditions durante la lettura di condizioni.xlsx: {e}")
...
def generate_forecasting_report(input_filepath, output_filepath, sheet_name="Sheet1"):
    ...
    conditions_path = "condizioni.xlsx"
    if os.path.exists(conditions_path):
        payment_conditions = get_payment_conditions(conditions_path)
    ...
    try:
        ...
        if payment_conditions_added: message += " con colonna 'Condizioni Pagamento'."
        return message
    except Exception as e:
